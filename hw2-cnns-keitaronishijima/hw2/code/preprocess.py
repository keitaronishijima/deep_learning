...
def get_data(file_path, first_class, second_class):
	...
	cat_only_label = np.nonzero(labels == first_class)
	dog_only_label = np.nonzero(labels == second_class)
	cat_and_dog_only_label = np.sort(np.concatenate((cat_only_label,dog_only_label)).reshape(-1))
	inputs = inputs[cat_and_dog_only_label]/255
	labels = labels[cat_and_dog_only_label]
	...
	return tf.cast(inputs, dtype = tf.float32), tf.cast(labels, dtype = tf.float32)